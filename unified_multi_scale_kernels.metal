...
// ============================================================================
// 2. VECTORIZED ELEMENTWISE KERNELS (RESIDUAL & SWIGLU & QUANTIZATION & TRANSPOSE)
// ============================================================================
...
// Layout Transpose: [M, H * D] -> [H, M, D] (handles any D divisible by 4)
...
// Layout Transpose: [H, M, D] -> [M, H * D] (handles any D divisible by 4)
...
// ============================================================================
// Arbitrary M, N, K (where K is a multiple of 32)
...
// ============================================================================
// Directly outputs Q, K, V in [H, M, D] layout for any H, D, K
...
// Eliminates intermediate DRAM roundtrips!
...
// ============================================================================
// 6A. FUSED FLASHATTENTION ENGINE FOR D=64 (1B Scale)
// ============================================================================

template<ushort TG_SIZE, ushort BC>
inline void load_kv_tile_fp16_d64(
    device const half* K,
    device const half* V,
...
template<ushort TG_SIZE, ushort BC>
inline void load_kv_tile_q8_0_d64(
...
// FlashAttention FP16 for D=64 (32x16 Tile)
kernel void flash_attn_fp16_causal_d64(
    device const half* Q     [[buffer(0)]], // [H, M, 64]
    device const half* K     [[buffer(1)]], // [H, M, 64]
    device const half* V     [[buffer(2)]], // [H, M, 64]
    device half*       O     [[buffer(3)]], // [M, H * 64]
    constant uint&     M     [[buffer(4)]],
    constant uint&     H     [[buffer(5)]],
    constant float&    scale [[buffer(6)]],
    threadgroup half*  shmem [[threadgroup(0)]], // [2][16*64 + 16*64] = 8KB
    ...
    uint tid     [[thread_index_in_threadgroup]])
{
    ...
    uint cur_buf = 0;
    if (loop_tiles > 0) {
        load_kv_tile_fp16_d64<TG_SIZE, BC>(K, V, smem_K[0], smem_V[0], h, 0, M, tid);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (uint b_c = 0; b_c < loop_tiles; b_c++) {
        ...
        if (next_b_c < loop_tiles) {
            load_kv_tile_fp16_d64<TG_SIZE, BC>(K, V, smem_K[nxt_buf], smem_V[nxt_buf], h, next_b_c * BC, M, tid);
        }
...
// FlashAttention Q8_0 for D=64 (32x16 Tile)
kernel void flash_attn_q8_0_causal_d64(
    device const half*       Q     [[buffer(0)]], // [H, M, 64]
    ...
    uint tid     [[thread_index_in_threadgroup]])
{
    ...
    uint cur_buf = 0;
    if (loop_tiles > 0) {
        load_kv_tile_q8_0_d64<TG_SIZE, BC>(K_q8, V_q8, smem_K[0], smem_V[0], h, 0, M, tid);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (uint b_c = 0; b_c < loop_tiles; b_c++) {
        ...
        uint next_b_c = b_c + 1;
        if (next_b_c < loop_tiles) {
            load_kv_tile_q8_0_d64<TG_SIZE, BC>(K_q8, V_q8, smem_K[nxt_buf], smem_V[nxt_buf], h, next_b_c * BC, M, tid);
        }
...
// ============================================================================
// 6B. FUSED FLASHATTENTION ENGINE FOR D=128 (8B & 16B Scales)
// ============================================================================
...
// FlashAttention FP16 for D=128 (32x16 Tile)
...
// FlashAttention Q8_0 for D=128 (32x16 Tile)

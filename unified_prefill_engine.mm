    ...
    CpuIntermediates* inter = nullptr)
{
    ...
    // Helper: Multithreaded CPU GEMM A[M, K] @ B_q4_0[K, N] -> C[M, N]
...
struct LayerProfile {
    ...
    double full_model_16l_ms;
};

// ============================================================================
// MAIN UNIFIED PREFILL ENGINE BENCHMARK
// ============================================================================
int main(int argc, const char* argv[]) {
    @autoreleasepool {
        std::cout << "==========================================================================================" << std::endl;
        std::cout << "        J.A.R.V.I.S. UNIFIED END-TO-END 1B PREFILL ENGINE BENCHMARK (APPLE M4)           " << std::endl;
        std::cout << "==========================================================================================" << std::endl;
        ...
        // --------------------------------------------------------------------
        // ARCHITECTURE CONSTANTS (LLaMA 1B STANDARD)
        // --------------------------------------------------------------------
        ...
        const uint32_t NUM_LAYERS = 16;  // Full 1B model layers

        std::cout << "[+] Architecture Configuration:" << std::endl;
        ...
        std::cout << "    - Full Model Extrapolation:      " << NUM_LAYERS << " Layers" << std::endl;
        ...
        id<MTLComputePipelineState> pso_flash_attn_q8_0     = load_pso(@"flash_attn_q8_0_causal");
        ...
        // --------------------------------------------------------------------
        // ALLOCATE MODEL WEIGHTS IN Q4_0 FORMAT
        // --------------------------------------------------------------------
        ...
        // ====================================================================
        std::cout << "\n==========================================================================================" << std::endl;
        std::cout << ">>> [STAGE 1] END-TO-END LAYER NUMERICAL VERIFICATION (GPU vs CPU REFERENCE)" << std::endl;
        std::cout << "==========================================================================================" << std::endl;

        const std::vector<uint32_t> verify_lengths = {128, 512, 1024};
        for (uint32_t M : verify_lengths) {
            std::cout << "\n--- Verifying Full Layer at Sequence Length M = " << M << " tokens ---" << std::endl;
            ...
            std::vector<__fp16> h_X_out_gpu_base(in_elements);
            ...
            // ----------------------------------------------------------------
            {
                ...
                // FlashAttention Q8_0 (64x32 Tile)
                ...
                MTLSize tg_size_64 = MTLSizeMake(64, 1, 1);
                [enc dispatchThreadgroups:grid_fa_q8 threadsPerThreadgroup:tg_size_64];
            ...
            eval_diff(h_X_out_gpu_opt, "Unified Engine (FP16 KV)");
            eval_diff(h_X_out_gpu_q8,  "Unified Engine (Q8_0 KV)");
        }

        std::cout << "\n[✓] 100% NUMERICAL ACCURACY CONFIRMED ACROSS ALL SUITES (MaxDiff <= 0.05)" << std::endl;

        // ====================================================================
        // STAGE 2: DETAILED BENCHMARKING & BREAKDOWN ACROSS ALL SEQUENCE LENGTHS
        // ====================================================================
        std::cout << "\n==========================================================================================" << std::endl;
        std::cout << ">>> [STAGE 2] END-TO-END PREFILL LAYER BENCHMARK & COMPONENT BREAKDOWN" << std::endl;
        std::cout << "==========================================================================================" << std::endl;
        ...
        struct SummaryRow {
            ...
            double full_16l_fp16_ms;
            double full_16l_q8_ms;
        };
        ...
        for (uint32_t M : benchmark_lengths) {
            std::cout << "\n------------------------------------------------------------------------------------------" << std::endl;
            std::cout << ">>> BENCHMARKING SEQUENCE LENGTH M = " << M << " TOKENS (Hidden K=" << K << ", H=" << H << ", D=" << D << ")" << std::endl;
            std::cout << "------------------------------------------------------------------------------------------" << std::endl;
            ...
            // ----------------------------------------------------------------
            // 1. BASELINE LAYER PIPELINE EXECUTION
            // ----------------------------------------------------------------
            auto run_baseline_pass = [&](bool measure_breakdown, LayerProfile& prof) {
                // Timer helpers
                ...
                prof.full_model_16l_ms = prof.total_ms * NUM_LAYERS;
            };
            ...
            auto run_unified_pass = [&](bool is_q8, LayerProfile& prof) {
                ...
                // Stage B: Fused FlashAttention Engine
                double t_attn = time_encoder(^(id<MTLComputeCommandEncoder> enc) {
                    if (!is_q8) {
                        [enc setComputePipelineState:pso_flash_attn_fp16];
                        [enc setBuffer:d_Q offset:0 atIndex:0];
            ...
            auto avg_prof = [&](LayerProfile& p) {
                ...
                p.full_model_16l_ms = p.total_ms * NUM_LAYERS;
            };
            ...
            print_row("EXTRAPOLATED 16L MODEL", prof_base.full_model_16l_ms, prof_opt_fp16.full_model_16l_ms, prof_opt_q8.full_model_16l_ms);

            std::cout << "\n    [*] Layer Throughput (tokens/sec):" << std::endl;
            std::cout << "        - Baseline Pipeline:      " << std::fixed << std::setprecision(0) << prof_base.throughput_tok_s << " tok/s" << std::endl;
            std::cout << "        - Unified FP16 Engine:    " << prof_opt_fp16.throughput_tok_s << " tok/s (" << std::setprecision(2) << (prof_opt_fp16.throughput_tok_s / prof_base.throughput_tok_s) << "x)" << std::endl;
            std::cout << "        - Unified Q8_0 Engine:    " << prof_opt_q8.throughput_tok_s << " tok/s (" << (prof_opt_q8.throughput_tok_s / prof_base.throughput_tok_s) << "x)" << std::endl;
            ...
            summary_table.push_back({
                ...
                prof_opt_fp16.full_model_16l_ms,
                prof_opt_q8.full_model_16l_ms
            });
        }
        ...
        // ====================================================================
        std::cout << "\n==========================================================================================" << std::endl;
        std::cout << "  EXECUTIVE SUMMARY: UNIFIED 1B PREFILL ENGINE vs BASELINE ON APPLE M4" << std::endl;
        std::cout << "==========================================================================================" << std::endl;
        std::cout << std::left << std::setw(10) << "Prompt M"
                  ...
                  << std::setw(16) << "Throughput FP16"
                  << std::setw(16) << "Throughput Q8"
                  << std::setw(16) << "Full 16L FP16" << std::endl;
        ...
        for (const auto& row : summary_table) {
            std::cout << std::left << std::setw(10) << row.M
                      ...
                      << std::setw(14) << row.tok_s_unified_q8 << " t/s"
                      << std::setw(13) << std::setprecision(2) << row.full_16l_fp16_ms << " ms" << std::endl;
        }

        std::cout << "==========================================================================================" << std::endl;
        std::cout << " [✓] ALL DELIVERABLE D5.1 OBJECTIVES AND BENCHMARKS COMPLETED SUCCESSFULLY." << std::endl;
        std::cout << "==========================================================================================" << std::endl;
    }
    return 0;
}
